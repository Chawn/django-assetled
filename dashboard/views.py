...
def get_html_content(a_type, a_province, a_ampur, page, bid_date, min_price,max_price): #search_url
    import requests
...
def isChanode(law_suit_no, law_suit_year,Law_Court_ID,deed_no): #search_url
    import requests
    ...
    # print(law_suit_no, law_suit_year,Law_Court_ID,deed_no)
    response = requests.get(f'http://asset.led.go.th/newbidreg/asset_open.asp?law_suit_no={law_suit_no}&law_suit_year={law_suit_year}&Law_Court_ID={Law_Court_ID}&deed_no={deed_no}&addrno=-')
    # print(law_suit_no, law_suit_year, Law_Court_ID, deed_no)
    soup = BeautifulSoup(response.content, 'html.parser')
    cardbody = soup.findAll("div", {"class": "card-body"})[0]
    cardbody_row = cardbody.findAll("div", {"class": "row"})[0]
    cardtext = cardbody_row.findAll("div", {"class": "card-text"})[4]
    print(f'deed_no: {deed_no}, cardtext: {cardtext}')
    chanode = re.findall("โฉนด", str(cardtext))
    print(f'chanode: {chanode}')
    if chanode:
        print('1')
        print(f'--------------------')
        return "1"
...
def Home(request):
    ...
    if 'province' in request.GET:
        ...
        while page_found==True:
            page += 1
            response = get_html_content(a_type, a_province, a_ampur, page, bid_date, min_price, max_price)
            ...
            for row in table.findAll("tr"):
                try:
                    ...
                    # law_suit_no=ผบ.903&law_suit_year=2556&Law_Court_ID=417&deed_no=3903&addrno=-'
                    ...
                    for index,column in enumerate(row.findAll("td")):
                        for data in column:
                            ...
                            data = data.replace(' ','')
                            if index==1 :
                                asset_dict[fields[index]] = deed_no
                    ...
                    #isChanode(law_suit_no,law_suit_year,law_court_id,deed_no)
                    mycol.insert_one(asset_dict) 
                    asset_dict = {}
                    # TODO: เพิ่มข้อมูลลง Mongo DB Collection
        ...
        for asset in Asset.objects.all():
            is_chanode = isChanode(asset.a_law_suit_no, asset.a_law_suit_year, asset.a_law_court_id, asset.a_deed_no)
            Asset.objects.filter( Q(a_law_suit = asset.a_law_suit) and Q(a_deed_no = asset.a_deed_no)).update(is_chanode = is_chanode)
            
            # Asset.objects.filter({
            #     "a_law_suit" : asset.a_law_suit,
            #     "a_deed_no" : asset.a_deed_no
            # }).update(is_chanode = is_chanode)
